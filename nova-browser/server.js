 ...
 * 注入到每个被代理页面中的运行时接管脚本
 * ------------------------------------------------------------------ */
/* Nova 前端改写 Service Worker：把代理页发出的"裸路径"请求（页内 JS 的
 * location.href='/xxx'、相对地址 fetch 等）在离开浏览器之前改写为
 * /?__nova_url=... 形态。
 * 动机：公网部署平台的入口 WAF 会按路径拦截部分裸路径请求（实测
 * /images/search 无论 query 一律 403 拦截页），而带 __nova_url 参数的
 * 形态放行；服务端兜底 302 只能救 WAF 放行的路径，救不了被拦的。
 * 目标站点自身的 SW 注册仍被注入脚本禁用（override 在我们注册之后）。 */
const NOVA_SW_JS = [
  '/* Nova Browser proxy rewrite service worker */',
  'var RESERVED=/^\\/$|^\\/index\\.html$|^\\/__|^\\/favicon\\.ico$/;',
  ...
  '  if(u.searchParams.has("__nova_url")||u.searchParams.has("__nova_sw"))return;',
  '  if(RESERVED.test(u.pathname))return;',
  '  var DBG={url:u.pathname.slice(0,40),ref:String(req.referrer||"").slice(0,60),cid:e.clientId||"",mode:req.mode};',
  '  function dbgp(tag){try{self.clients.matchAll({type:"window",includeUncontrolled:true}).then(function(ls){for(var i=0;i<ls.length;i++){try{ls[i].postMessage({__nova_dbg:{tag:tag,url:DBG.url,ref:DBG.ref}});}catch(_){}}});}catch(_){}}',
  '  dbgp("enter");',
  '  /* 来源探测三级回退：referrer → 当前 client → 任意窗口 client。',
  '   * 总是 respondWith，保证裸路径请求绝不原样出网（WAF 会按路径拦截）。 */',
  '  e.respondWith((function(){',
  '    function pick(urls){',
  '      for(var i=0;i<urls.length;i++){',
  '        try{var p=new URL(urls[i]).searchParams.get("__nova_url");if(p)return new URL(p).origin;}catch(_){}',
  '      }',
  '      return null;',
  '    }',
  '    var seeds=[];',
  '    if(req.referrer)seeds.push(req.referrer);',
  '    return self.clients.matchAll({type:"window",includeUncontrolled:true}).then(function(list){',
  '      for(var i=0;i<list.length;i++)seeds.push(list[i].url);',
  '      var o=pick(seeds);',
  '      dbgp("matchall n="+list.length+" origin="+(o||"null"));',
  '      if(!o)return fetch(req);',
  '      return Response.redirect(self.location.origin+"/?__nova_url="+encodeURIComponent(o+u.pathname+u.search+u.hash)+"&__nova_sw=1",302);',
  '    }).catch(function(err){dbgp("matchall-err:"+err);return fetch(req);});',
  '  })());',
  '});',
].join('\n');
...
function buildInjectScript(realUrl) {
  ...
  return [
    ...
    '  var s=u.search;',
    '  /* 公网平台 WAF 按路径拦截（如 /images/search），与参数无关：风险路径走根路径形态 */',
    '  if(/\\/images\\/search(\\/|$)/i.test(u.pathname))return "/?__nova_url="+encodeURIComponent(u.href);',
...
'  window.postMessage=__nat(function(m,t){try{ckFlushNow();}catch(e){}try{var tg="";try{tg=(t&&t.origin)||String(t);}catch(e){}nlog("PM_OUT to="+tg+" d="+String(typeof m==="string"?m:(m&&m.type)||Object.prototype.toString.call(m)).slice(0,240));}catch(e){}try{var ar=[].slice.call(arguments);if(ar.length)ar[0]=fixPm(ar[0]);return _pm.apply(window,ar);}catch(e){}return _pm.apply(window,arguments);},_pm);',
    ...
    '  }',
    '}catch(e){}',
    /* Nova 自装 SW：把本域"裸路径"请求在出浏览器前改写成 /?__nova_url=... 形态
     * （公网平台 WAF 按路径拦截部分裸路径，如 /images/search）。
     * 必须在下方禁用 register 的 override 之前调用原生方法。 */
    'try{',
    '  var _sw=navigator.serviceWorker;',
    '  if(_sw&&_sw.register&&(location.protocol==="https:"||location.hostname==="127.0.0.1"||location.hostname==="localhost")){',
    '    _sw.register(PROXY+"/__nova_sw.js",{scope:"/"}).catch(function(){});',
    '  }',
    '}catch(e){}',
...
function proxyUrlFor(realUrl) {
  ...
  const s = u.search;
  /* 公网部署平台的入口 WAF 按路径拦截部分请求（与 query 无关，实测
   * /images/search 一律 403 拦截页）。这些路径改用根路径形态（实测放行）。
   * 若平台后续新增拦截路径，扩充此正则即可。 */
  if (/\/images\/search(\/|$)/i.test(u.pathname)) {
    return '/?__nova_url=' + encodeURIComponent(u.href);
  }
  return u.pathname + s + (s ? '&' : '?') + '__nova_url=' + encodeURIComponent(u.href);
}
...
function start(port) {
  const server = http.createServer((req, res) => {
    ...
    if (p !== '/' && p !== '/index.html' && !p.startsWith('/__')) {
      ...
      if (rOrigin) {
        /* 302 目标必须用根路径形态 /?__nova_url=...：公网平台 WAF 的路径拦截
         * 规则（如 /images/search）与参数无关，同路径+参数的重定向目标会被再次
         * 拦截；根路径形态实测放行。 */
        res.writeHead(302, { location: '/?__nova_url=' + encodeURIComponent(rOrigin + p + parsed.search) + '&__nova_swfb=1' });
        return res.end();
      }
    }
